...
from torch.cuda import amp  # For automatic mixed precision training
...
# Project-specific dataset and config management
...
def parse_args():
    """Parse all command-line arguments for configuring the training process."""
    parser = argparse.ArgumentParser(description='Train Multitask network')
    # Optional experiment directories
    ...
    parser.add_argument(
        '--seg-only',
        action='store_true',
        help='only train drivable‐area & lane‐segmentation (no detection)'
    )

    # Distributed training & batch norm
...
def main():
    # Parse all args and update configuration
    args = parse_args()
    ...
    # cuDNN backend settings for performance and reproducibility
    cudnn.benchmark = cfg.CUDNN.BENCHMARK
    torch.backends.cudnn.deterministic = cfg.CUDNN.DETERMINISTIC
    torch.backends.cudnn.enabled = cfg.CUDNN.ENABLED

    # Build and allocate the model
    print("begin to bulid up model...")
    device = select_device(logger, batch_size=cfg.TRAIN.BATCH_SIZE_PER_GPU * len(cfg.GPUS)) if not cfg.DEBUG \
        else select_device(logger, 'cpu')

    # Distributed: set device and initialize process group if needed
    ...
    # Index lists for freezing/unfreezing specific model heads
    ...
    # Lambda-based cosine learning rate schedule
    lf = lambda x: ((1 + math.cos(x * math.pi / cfg.TRAIN.END_EPOCH)) / 2) * \
                   (1 - cfg.TRAIN.LRF) + cfg.TRAIN.LRF  # cosine
    lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lf)
    begin_epoch = cfg.TRAIN.BEGIN_EPOCH

    # ----- Checkpoint and branch-specific pretraining logic -----
    if rank in [-1, 0]:
        ...
        # Freeze/unfreeze specific layers according to config flags
    ...
    # DataParallel (DP) for single-machine, multi-GPU training
    if rank == -1 and torch.cuda.device_count() > 1:
        model = torch.nn.DataParallel(model, device_ids=cfg.GPUS)
    # DistributedDataParallel (DDP) for multi-process distributed training
    if rank != -1:
        model = DDP(model, device_ids=[args.local_rank], output_device=args.local_rank, find_unused_parameters=True)

    # Assign model params for custom heads
    model.gr = 1.0
    model.nc = 1

    print("begin to load data")
    # Data normalization for pre-trained models (e.g., ImageNet)
    normalize = transforms.Normalize(
        mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
    )

    # Train dataset and DataLoader (with DDP sampler if needed)
    ...
    # Validation dataset/loader only in main process
    if rank in [-1, 0]:
    ...
    # Check or generate anchors if required by the detection head
    if rank in [-1, 0]:
    ...
    for epoch in range(begin_epoch + 1, cfg.TRAIN.END_EPOCH + 1):
        ...
        # Evaluate on validation set at configured frequency
        if (epoch % cfg.TRAIN.VAL_FREQ == 0 or epoch == cfg.TRAIN.END_EPOCH) and rank in [-1, 0]:
            ...
            fi = fitness(np.array(detect_results).reshape(1, -1))  # Fitness metric

            # Log detailed epoch stats
        ...
        # Save checkpoint for every epoch and always keep 'checkpoint.pth' updated
        if rank in [-1, 0]:
